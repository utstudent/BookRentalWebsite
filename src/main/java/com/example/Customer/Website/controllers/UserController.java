...
public class UserController {
    ...
    public String saveUser(@ModelAttribute("user") User user) {
        ...
        User newuser1 = User.builder().username(user.getUsername()).password(passwordEncoder.encode(user.getPassword())).authorities(Arrays.asList(Role.builder().role(Role.Roles.ROLE_USER.name()).build())).build();
